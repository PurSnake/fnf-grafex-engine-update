...
class Alphabet extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1, ?white:Bool = false)
	{
		super(x, y);
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacter = new AlphaCharacter(30 * loopNum, 0, textSize, isWhite);
				var letter:AlphaCharacter = new AlphaCharacter(xPos, 0, textSize, isWhite);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacter = new AlphaCharacter(30 * loopNum, 0, textSize, isWhite);
				var letter:AlphaCharacter = new AlphaCharacter(xPos, 55 * yMulti, textSize, isWhite);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			{
				case "Classic":
					y = FlxMath.lerp(y, (scaledY * yMult) + (FlxG.height * 0.48), 0.16/(ClientPrefs.framerate / 60));
				    x = FlxMath.lerp(x, (targetY * 20) + 90, 0.16/(ClientPrefs.framerate / 60));
				case "Vertical":
					y = FlxMath.lerp(y, (scaledY * yMult) + (FlxG.height * 0.5), 0.16/(ClientPrefs.framerate / 60));
...
class AlphaCharacter extends FlxSprite
{
	...
	public function new(x:Float, y:Float, textSize:Float, ?white:Bool = false)
	{
		...
		var tex = Paths.getSparrowAtlas('alphabet');
                if(white) tex = Paths.getSparrowAtlas('alphabet_white');

...
class FunkinLua {
	...
	public function new(script:String) {
		...
		var difficultyName:String = Utils.difficulties[PlayState.storyDifficulty];
		...
		Lua_helper.add_callback(lua, "preCashe", function(key:String, type:String) {
			switch(type)
			{
				case 'image':
					Paths.image(key);
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = false) {
			...
			if(PlayState.isStoryMode)
				grafex.system.statesystem.MusicBeatState.switchState(new StoryMenuState());
			else
				grafex.system.statesystem.MusicBeatState.switchState(new FreeplayState());
		...
		Lua_helper.add_callback(lua, "close", function(name:String, legacy:Bool) {
			var status:Bool = PlayState.instance.closeScript(name);
			(status) ? trace('lua script closed: $name') : trace('no lua script to close: $name');
			return status;
		});
		...
		Lua_helper.add_callback(lua, "initSaveData", function(name:String, ?folder:String = 'grafexmods') {
		...
		Lua_helper.add_callback(lua, "gradeAchievement", function(name:String) {
                        AchievementsGrfx.setAchievement(name, true);
			return true;
		});

		Lua_helper.add_callback(lua, "getAchievement", function(name:String) {
			return AchievementsGrfx.getAchievement(name);
		});

                #if DEVS_BUILD
		Lua_helper.add_callback(lua, "setAchievement", function(name:String, ?value:Bool = true) {

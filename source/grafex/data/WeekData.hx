...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false, ?oddPath:String = '')
	{
		weeksList = [];
		weeksLoaded.clear();
		#if MODS_ALLOWED
		...
		{
			var stuff:Array<String> = Utils.coolTextFile(modsListPath);
		...
		var sexList:Array<String> = Utils.coolTextFile(Paths.getPreloadPath('weeks/weekList.txt'));
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories[j] + 'weeks/' + oddPath + sexList[i] + '.json';
		...
		for (i in 0...directories.length) {
			var directory:String = directories[i] + 'weeks/' + oddPath;
			if(FileSystem.exists(directory)) {
				var listOfWeeks:Array<String> = Utils.coolTextFile(directory + 'weekList.txt');

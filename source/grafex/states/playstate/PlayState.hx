...
class PlayState extends MusicBeatState
{
    ...
    public static var smoothCamera:Bool = true;
    ...
    public var canComboShit:Bool = false;
	var delta:Float;
	...
	override public function create()
	{
		...
		Application.current.window.title = Main.appTitle + ' // ' + StringTools.replace(SONG.song, '-', ' ');   //i should rewrite getArtist in future - PurSnake
	...
	boyfriend = new Character(0, 0, SONG.player1, true);
        ...
        for (object in [strumLineNotes, grpNoteSplashes, notes, healthBar, healthBarWN, healthBarBG, iconP1, iconP2, scoreTxt, botplayTxt, doof, sub]) object.cameras = [camHUD];
        ...
        if(ret != FunkinLua.Function_Stop) {
            ...
            startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
            {
		        for (character in characters) {
			        if (character!=null && tmr.loopsLeft%Math.round(character.danceEveryNumBeats*(character==gf?gfSpeed:1))==0 && character.animation.curAnim!=null && !character.animation.curAnim.name.startsWith("sing") && !character.stunned){
			        	character.dance();
			        }
		        }
    ...
    function startSong():Void
    {
		...
		FlxTween.tween(timeBar, {alpha: .9}, 0.3, {ease: FlxEase.circOut});
		FlxTween.tween(timeBarBG, {alpha: .4}, 0.3, {ease: FlxEase.circOut});
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			...
			case 'FreeStyleImageFUUUCK': 
				Paths.image('solo_time');
				Paths.sound('spray_sound');
			}
	...
	function eventNoteEarlyTrigger(event:EventNote):Float {
		...
		callOnHscript("eventEarlyTrigger", [event.event]);
	}
    ...
    {
        #if desktop
        if (health > 0 && !paused) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconRPC, SONG.player2);
        #end
    
        if (!FlxG.autoPause && !paused && canPause && startedCountdown && !cpuControlled) pauseState();
	...
	function checkFutureNotes(time:Float)
	{
		...
		while (i <= notes.length - 1 && notes.length > 0) {
			var daNote:Note = notes.members[i];
			if(daNote.mustPress && daNote.strumTime - (Conductor.crochet * 3.75 * playbackRate) > time) {
				notesExist=true;
				break;
			}
			++i;
		...
		switch(eventName) {
			...
			case 'Dialogue':
				...
				if (value2 != null && value2.length > 0 && value2 != '' && value2 != ' ')
		            ...
		            if(value3 != null && value3 != '' && value3 != ' ') sub.size = Std.parseInt(value3);
           
		            if(value1 != null && value1.length > 0 && sub.alpha < 1) sub.alpha = 1;
       
		            sub.screenCenter();
		            sub.y += 170;
		            if(value1 == null || value1 == '') sub.alpha = 0;
	...
	public function endSong():Void
	{
        ...
        popUpComboScore(comboScore, true);
	...
	private function popUpScore(note:Note = null):Void
    {
        ...
        for (i in seperatedScore)
        {
            ...
            if(numSpriteType != 'animated') {
                if (!PlayState.isPixelStage)
                    numScore.setGraphicSize(Std.int(numScore.width * (50/numScore.width)));
                else
        ...
        if (combo >= 65) insert(members.indexOf(strumLineNotes), comboSpr);
			...
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				if (sortedNotesList.length > 0) {
					...
					if (canMiss) {
						...
						callOnHscript("noteMissPress", [key]);
                        stageBuild.callFunction('noteMissPress', [key]);
					}
    ...
    function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
    {
		...
		dad.specialAnim = true;
            ...
            if(/*ClientPrefs.playMissAnims &&*/ boyfriend.hasMissAnimations) boyfriend.playAnim(singAnimations[direction] + 'miss', true);
    ...
    function goodNoteHit(note:Note):Void
    {
        if (!note.wasGoodHit)
        {
           ...
           //comboTimer = new FlxTimer().start(Conductor.crochet / 250 / playbackRate, function(tmr:FlxTimer) { comboTimer = null; popUpComboScore(comboScore, comboNum ?); });
    ...
    public function popUpComboScore(cscore:Int, ?miss:Bool = false)
    {
	...
	function spawnComboSprites(comboCol:Int) {
        ...
        for (i in seperatedScore)
        {
		    ...
		    if(funnyNum.frames != null) {
				...
				FlxTween.tween(funnyNum, {x: funnyNum.x + 62, y: funnyNum.y - 31},  (funnyNum.animation.curAnim.frames.length - 3)/12 , {ease: FlxEase.quadOut});
				var timer:FlxTimer = new FlxTimer().start(funnyNum.animation.curAnim.frames.length/12, function(tmr:FlxTimer)
				{   funnyNum.kill();
					remove(funnyNum, true);
				});

				modchartTimers.set('funnyNumber'+i, timer);
		    }
		...
		var addTimer:FlxTimer = new FlxTimer().start(Conductor.crochet / 8000 / playbackRate, function(tmr:FlxTimer)
		{
            ...
            comboSprite.x -= 62;
            comboSprite.y += 31;
            FlxTween.tween(comboSprite, {x: comboSprite.x + 62, y: comboSprite.y - 31},  (comboSprite.animation.curAnim.frames.length - 4)/14 , {ease: FlxEase.quadOut});
		    var timer:FlxTimer = new FlxTimer().start(comboSprite.animation.curAnim.frames.length/14, function(tmr:FlxTimer)
		    {
		    	comboSprite.kill();
		    	remove(comboSprite, true);
		    });
			modchartTimers.set('funnyComboSprite', timer);
		});
	...
	public function spawnNoteSplashOnNote(note:Note, ?char:String = 'bf')
    {
        ...
        if(ClientPrefs.noteSplashes && note != null)
        {
            ...
            if(strum != null && strum.alpha > 0.4) spawnNoteSplash(strum.getMidpoint().x, strum.getMidpoint().y, note.noteData, note);
        }
    }
	...
	override function beatHit()
	{
		...
		for (character in characters){
			if (character!=null && curBeat % Math.round(character.danceEveryNumBeats*(character==gf?gfSpeed:1))==0 && character.animation.curAnim!=null && !character.animation.curAnim.name.startsWith("sing") && !character.stunned){
				character.dance();
			}
		}
	...
	override function sectionHit()
    {
        ...
        if (camZooming && ClientPrefs.camZooms && camZoomingFreq == 0)
        {
            FlxG.camera.zoom += 0.015 * camZoomingMult;
            camHUD.zoom += 0.03 * camZoomingMult;
        }

		if(curSection > 1 && SONG.notes[curSection-1].mustHitSection && !SONG.notes[curSection].mustHitSection && !checkFutureNotes(Conductor.songPosition)) popUpComboScore(comboScore, comboNum <=6);
	...
	public function callOnHscript(eventName:String, args:Array<Dynamic>, ignoreStops = true, exclusions:Array<String> = null):Dynamic {
		...
		if (hscriptArray.length > 0)
			for (script in hscriptArray) {
				smthVal = script.executeFunc(eventName, args);
			}
		return smthVal;
	}
	...
	public function setOnLuas(variable:String, arg:Dynamic)
	{
		...
		//if(stageBuild.exist) stageBuild.set(variable, arg);
	}
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			callOnHscript("onRecalculateRating", []);
	...
	public function startVideo(name:String):Void
	{
		...
		var fileName:String = #if MODS_ALLOWED Paths.video(name); #else ''; #end
		#if sys

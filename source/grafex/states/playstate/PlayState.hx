...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		Application.current.window.title = Main.appTitle + ' // ' + StringTools.replace(SONG.song, '-', ' ') + ' ['+storyDifficultyText.toUpperCase()+']'+ (PlayState.SONG.composedBy != '' ? ' - ' + PlayState.SONG.composedBy : ''); 
		...
		if(PlayState.isPixelStage) introSoundsSuffix = '-pixel';
			...
			if(FileSystem.exists(folder))
			{
				for (file in FileSystem.readDirectory(folder))
				{
					if(!filesPushed.contains(file)){
						if(file.endsWith('.lua')) luaArray.push(new FunkinLua(folder + file));
						if(file.endsWith('.hx')) hscriptArray.push(GrfxScriptHandler.noPathModule(folder + file));
	...
	boyfriend = new Character(0, 0, SONG.player1, true);
		...
		if(ClientPrefs.downScroll) timeBarBG.y = FlxG.height - 8;
		...
		if(ClientPrefs.downScroll) timeBar.y = FlxG.height - 8;
		...
		for (noteType in noteTypeMap.keys()) loadScript("notetypes", noteType);

		for (event in eventPushedMap.keys()) loadScript("events", event);
		...
		healthBarGroup = new FlxTypedGroup<Dynamic>();
			...
			healthBarWN = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBar.height / 2), this, 'displayedHealth', 0, 2);
		...
		iconP1 = new HealthIcon(boyfriend.healthIcon, true, boyfriend.iconOffsets[0], boyfriend.iconOffsets[1], boyfriend.iconScale);
		iconP1.y = healthBar.y - (iconP1.height / 2);
		iconP1.visible = !ClientPrefs.hideHud;
		iconP1.alpha = 1;

		iconP2 = new HealthIcon(dad.healthIcon, false, dad.iconOffsets[0], dad.iconOffsets[1], dad.iconScale);
		...
		else if (skipCountdown && skippedCountdownStartTime < 0)
		{
            ...
            startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
            {
		        for (character in characters) {
			        if (character!=null && tmr.loopsLeft%Math.round(character.danceEveryNumBeats*(character==gf?gfSpeed:1))==0 && character.animation.curAnim!=null && !character.animation.curAnim.name.startsWith(character.singAnimsPrefix) && !character.stunned){
			        	character.dance();
			        }
		        }
                ...
                iconGroup.forEach(function(icon:HealthIcon)
                {
                    ...
                    stageBuild.callFunction("onIconsBeatAnim", [swagCounter]);
                    callOnLuas('onIconsBeatAnim', [swagCounter]);
                });
            ...
            for (songNotes in section.sectionNotes) {
                ...
                if(floorSus > 0) {
                    ...
                    for (susNote in 0...floorSus) {
                        ...
                        var sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true);
	...
	function eventNoteEarlyTrigger(event:EventNote):Float
	{
		...
		if (callOnHscript('eventEarlyTrigger', [event.event]) != 0) returnedValue = callOnHscript('eventEarlyTrigger', [event.event]);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (ClientPrefs.middleScroll && player < 1) targetAlpha = 0.35;
	...
	override public function onFocus():Void
    {
        ...
        {
            if (Conductor.songPosition > 0.0) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC, SONG.player2, true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
            else DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC, SONG.player2);
        }
        #end
	...
	override public function update(elapsed:Float)
    {
		...
		if (SONG.needsVoices && updateTime && vocals.volume < 1) vocals.volume += 0.6 *elapsed;
        ...
        if(smoothCamera) {
            if((1 - ClientPrefs.framerate * 0.008) > 0) FlxG.camera.followLerp =  1 - (ClientPrefs.framerate * 0.008);
            else FlxG.camera.followLerp =  0.04;
        } else FlxG.camera.followLerp = 60 / FlxG.updateFramerate;
		...
		{
			var ret:Dynamic = (callOnLuas('onPause', [], false) || callOnHscript('onPause', [], false));
			if(ret != FunkinLua.Function_Stop) {
				pauseState();
			}
		}
	    ...
	    if(isHealthCheckingEnabled)
            iconGroup.forEach(function(icon:HealthIcon)
            {
                icon.isPlayer ? icon.updateAnim(healthBar.percent) : icon.updateAnim(100 - healthBar.percent);
            });	

        iconGroup.forEach(function(icon:HealthIcon)
        {
            icon.doIconPos(elapsed * iconsZoomingDecay);
        });
		...
		if (generatedMusic && !inCutscene)
		{
			if(!cpuControlled) {
				keysCheck();
			} else if(boyfriend.animation.curAnim != null && boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * boyfriend.singDuration && boyfriend.animation.curAnim.name.startsWith(boyfriend.singAnimsPrefix) && !boyfriend.animation.curAnim.name.endsWith('miss')) {
				boyfriend.dance();
			}
	...
	function loadScript(scriptType:String, scriptName:String){
		...
		if (FileSystem.exists(luaTL) || OpenFlAssets.exists(luaTL)) luaArray.push(new FunkinLua(luaTL));

		if (FileSystem.exists(hxTL) || OpenFlAssets.exists(hxTL)) hscriptArray.push(GrfxScriptHandler.noPathModule(hxTL));
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				!ClientPrefs.instantRespawn ? openSubState(new GameOverSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0], boyfriend.getScreenPosition().y - boyfriend.positionArray[1]))
	...
	{
		ClientPrefs.classicScoreTxt ? scoreTxt.text = "Score:" + songScore : scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName + (ratingName != '?' ? ' (${Highscore.floorDecimal(ratingPercent * 100, 2)}%) - $ratingFC' : '');
        ...
        stageBuild.callFunction('onUpdateScore', [miss]);
	}
    public function triggerEventNote(eventName:String, value1:String, value2:String, ?value3:String) {
		switch(eventName) {
			...
			case 'Sing Animation Prefix':
				...
				if (value2 == null || value2 == '' || value2 == ' ') value2 = 'sing';
			...
			case 'Dialogue':
				...
				if (value2 != null && value2.length > 0 && value2 != '' && value2 != ' ')
				    ...
				    if (value1 != null && value1 != '') sub.applyMarkup(value1, [subMark]);

		            if(value1 != null && value1.length > 0 && sub.alpha < 1) {
			        if(subtitlesTxtAlphaTween != null) subtitlesTxtAlphaTween.cancel();
                                sub.alpha = 1;
                            }
		            ...
		            if(value3 != null && value3 != '' && value3 != ' ') {
                                ...
                                if(props.length > 0) sub.size = Std.parseInt(props[0]);

                                if(props.length > 1) sub.alpha = props[1];
                            }
           
		            if(value1 != null && value1.length > 0 && sub.alpha < 1) sub.alpha = 1;
       
		            sub.screenCenter();
		            sub.y += 170;

		            if(value1 != null && value1.length > 0) {   
			        if(subtitlesTxtTween != null) subtitlesTxtTween.cancel();
	...
	function moveCameraSection():Void 
	{
        ...
        callOnLuas('onPreMoveCamera', []);
        callOnHscript("onPreMoveCamera", []);
        stageBuild.callFunction('onPreMoveCamera', []);
	...
	var score:Float = 350;
        ...
        for (i in seperatedScore)
        {
            ...
            if(numSpriteType != 'animated') {
                if (!PlayState.isPixelStage)
                    numScore.setGraphicSize(Std.int(numScore.width * (54/numScore.width)));
                else
                    numScore.setGraphicSize(Std.int(numScore.width * daPixelZoom)); 
            }
            else numScore.scale.set(animatedNumsScale, animatedNumsScale);
		...
		switch(comboSpriteType) {
			...
			case 'default':     comboSpr.loadGraphic(Paths.image(ratingsSubPath + pixelShitPart1 + 'combo' + pixelShitPart2));
            case 'animated':    comboSpr.frames = Paths.getSparrowAtlas(ratingsSubPath + pixelShitPart1 + 'combo' + pixelShitPart2);
        ...
        if (combo >= 65) insert(members.indexOf(strumLineNotes)-1, comboSpr);
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if(!canShitButtons) return; //nuh uh

		if (!ClientPrefs.controllerMode && FlxG.keys.checkStatus(eventKey, JUST_PRESSED)) keyPressed(key);
	}
	...
	private function keysCheck():Void
	{
		...
		if(ClientPrefs.controllerMode || strumsBlocked.contains(true))
		{
			...
			if(parsedArray.contains(true))
			{
				for (i in 0...parsedArray.length)
				{
					if(parsedArray[i] || strumsBlocked[i] == true) onKeyRelease(new KeyboardEvent(KeyboardEvent.KEY_UP, true, true, -1, keysArray[i][0]));
				}
			}
		}
	}
	...
	{ //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if ((camFocus == 'bf' || camFocus == 'gf') && ClientPrefs.shouldCameraMove && (!daNote.noAnimation || daNote.specialNote))
	        ...
	        callOnHscript("noteMiss", [notes.members.indexOf(daNote), daNote.noteData, daNote.noteType, daNote.isSustainNote, daNote.ID]);
                stageBuild.callFunction('noteMiss', [notes.members.indexOf(daNote), daNote.noteData, daNote.noteType, daNote.isSustainNote, daNote.ID]);
	}
    ...
    function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
    {
		...
		dad.specialAnim = true;
            ...
            if(/*ClientPrefs.playMissAnims &&*/ boyfriend.hasMissAnimations) boyfriend.playAnim(boyfriend.singAnimsPrefix+singAnimations[Std.int(Math.abs(direction))] + 'miss', true);
    
            if(ClientPrefs.playMissSounds) vocals.volume = 0;
        }
    ...
    private function opponentNoteHit(note:Note):Void
    {
        ...
        if (!note.isSustainNote && FlxG.random.bool(50)) spawnNoteSplashOnNote(note, 'dad');
        ...
        if (SONG.needsVoices) {
		vocals.volume = 1;
		if (vocals2.length > 1) vocals2.volume = 1;
	}

        StrumPlayAnim(true, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate, (note.isSustainNote && !note.animation.curAnim.name.endsWith('end')));
    ...
    function goodNoteHit(note:Note):Void
    {
        if (!note.wasGoodHit)
        {
            ...
            if(cpuControlled) {
                StrumPlayAnim(false, leData, Conductor.stepCrochet * 1.25 / 1000 / playbackRate, (isSus && !note.animation.curAnim.name.endsWith('end')));
            } else {
		StrumPlayAnim(false, leData, Conductor.stepCrochet * 2 / 1000 / playbackRate, (isSus && !note.animation.curAnim.name.endsWith('end')));
            }
	...
	{
		//songScore+=cscore;
		//var scoreBonus:Int = comboNum * (comboScore / 100);

		if (!miss) spawnComboSprites(comboNum);

		if(forced || !miss) {
			var bonusScore:Int = Math.round(comboNum * (comboScore / 100));
			songScore += Math.round(bonusScore - (comboScore / 100));

			//songScore += Std.int(comboNum * (comboScore / 100)); // bonus score by doing big combo - PurSnake
		}
	...
	function spawnComboSprites(comboCol:Int) {
		...
		var daLoop:Int = 0;
		    ...
		    if(funnyNum.frames != null) {
		    	...
		    	modchartTweens.set('funnyNumTween'+i, FlxTween.tween(funnyNum, {x: funnyNum.x + 62, y: funnyNum.y - 31},  (funnyNum.animation.curAnim.frames.length - 3)/12 , {ease: FlxEase.quadOut}));
		...
		var addTimer:FlxTimer = new FlxTimer().start(Conductor.crochet / 8000 / playbackRate, function(tmr:FlxTimer)
		{
			if(comboSprite.frames != null) {
				...
				comboSprite.x -= 62;
				comboSprite.y += 31;
				modchartTweens.set('comboSpriteTween', FlxTween.tween(comboSprite, {x: comboSprite.x + 62, y: comboSprite.y - 31},  (comboSprite.animation.curAnim.frames.length - 4)/14 , {ease: FlxEase.quadOut}));
	...
	{
		if(char == 'dad' && !ClientPrefs.lightCpuStrums) return;

		if(ClientPrefs.noteSplashes && note != null)
		{
			var strum:StrumNote = playerStrums.members[note.noteData];
			if(char == 'dad') strum = opponentStrums.members[note.noteData];

			if(strum != null && strum.alpha > 0.4 && strum.visible) spawnNoteSplash(strum.getMidpoint().x, strum.getMidpoint().y, note.noteData, note);
		}
	}
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if (note != null) splash.scrollFactor.set(note.scrollFactor.x, note.scrollFactor.y);
		grpNoteSplashes.add(splash);
	}
	...
	override function beatHit()
	{
        ...
        iconGroup.forEach(function(icon:HealthIcon)
		{
            ...
            stageBuild.callFunction('onIconsBeatAnim', [curBeat]);
            callOnLuas('onIconsBeatAnim', [curBeat]);
		});
		...
		for (character in characters){
			if (character!=null && curBeat % Math.round(character.danceEveryNumBeats*(character==gf?gfSpeed:1))==0 && character.animation.curAnim!=null && !character.animation.curAnim.name.startsWith(character.singAnimsPrefix) && !character.stunned){
				character.dance();
			}
		}
	...
	if(curSection > 1 && SONG.notes[curSection] != null && SONG.notes[curSection-1].mustHitSection && !SONG.notes[curSection].mustHitSection && !checkFutureNotes(Conductor.songPosition)) popUpComboScore(comboNum <=6, true);
	...
	public function callOnHscript(eventName:String, args:Array<Dynamic>, ignoreStops = true, exclusions:Array<String> = null):Dynamic {
		var returnVal:Dynamic = FunkinLua.Function_Continue;
		if(exclusions == null) exclusions = [];
		for (script in hscriptArray) {
			if(exclusions.contains(script.scriptName))
			...
			var ret:Dynamic = script.executeFunc(eventName, args);
			if(ret == FunkinLua.Function_StopLua && !ignoreStops)
				break;
	...
	public function closeScript(name:String):Bool {
		var dick:Bool = false;
		#if LUA_ALLOWED
		for (script in luaArray) if (script.scriptName == name) dick = script.close();
		#end
		for (script in hscriptArray) if (script.scriptName == name) dick = script.dispose();
		return dick;
	}
	...
	public function setOnLuas(variable:String, arg:Dynamic, exclusions:Array<String> = null)
	{
		...
		#end
		for (script in hscriptArray) script.set(variable, arg);
	}
	...
	function StrumPlayAnim(isDad:Bool, id:Int, time:Float, ?isSus:Bool = false) {
		...
		if(isDad) {
			if (ClientPrefs.lightCpuStrums) spr = strumLineNotes.members[id];
		} else {
	...
	public function spawnSusNoteSplash(x:Float, y:Float, data:Int, ?strum:StrumNote = null) {
		...
		if (strum != null) splash.scrollFactor.set(strum.scrollFactor.x, strum.scrollFactor.y);
		grpSusSplashes.add(splash);
	}

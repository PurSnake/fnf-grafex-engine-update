...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Sing Animation Prefix', "Sets a specified prefix before the sing animation name.\nYou can use this to trigger 'singUP' if you set\nValue 2 to sing\n\nValue 1: Character to set (Dad, BF or GF)\nValue 2: New prefix (Leave it blank to default)"],
		...
		['Dialogue', "Value 1: Text to show\nMarkup color in 'RRGGBB' format\n \n \nExample: FNF is <m>NOT COOL<m>!"],
...
var directories:Array<String> = [Paths.mods('stages/'), Paths.mods(Paths.currentModDirectory + '/stages/'), Paths.getPreloadPath('stages/')];
		...
		tempMap.clear();
		var stageFile:Array<String> = Utils.coolTextFile(Paths.txt('stageList'));
		...
		difficultyDropDown.selectedLabel = Utils.difficulties[PlayState.storyDifficulty];
		...
		tab_group_song.add(UI_songTitlePostfix);
		...
		tab_group_song.add(noteExtraInputText);
		...
		tab_group_song.add(new FlxText(noteExtraInputText.x, noteExtraInputText.y - 15, 0, 'Extra Texture:'));
	...
	function addChartingUI() {
		...
		voicesVolume2 = new FlxUINumericStepper(voicesVolume.x + 100, voicesVolume.y, 0.1, 1, 0, 1, 1);
		voicesVolume2.value = vocals2.volume;
		...
		tab_group_chart.add(voicesVolume2);
		tab_group_chart.add(check_mute_inst);
		tab_group_chart.add(shiftNoteDialLabel);
		tab_group_chart.add(stepperShiftNoteDial);
        tab_group_chart.add(shiftNoteDialLabel2);
        tab_group_chart.add(stepperShiftNoteDialstep);
        tab_group_chart.add(shiftNoteDialLabel3);
        tab_group_chart.add(stepperShiftNoteDialms);
	...
	override function update(elapsed:Float)
	{
		...
		_song.postfix = UI_songTitlePostfix.text;
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				FlxG.sound.playMusic(Paths.music('animaniaLOOP'));
				return;
			}
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
							...
							FlxG.sound.play(Paths.sound(soundToPlay),0.7).pan = note.noteData < 4? -0.3 : 0.3; //would be coolio
	...
	function updateWaveform() {
		...
		if (FlxG.save.data.chart_waveformVoices2) {
			var sound:FlxSound = vocals2;
			if (sound._sound != null && sound._sound.__buffer != null) {
				var bytes:Bytes = sound._sound.__buffer.data.toBytes();
	...
	function setupSusNote(note:Note, beats:Float):FlxSprite {
		...
		var colorList:Array<FlxColor> = [FlxColor.PURPLE, FlxColor.CYAN, FlxColor.GREEN, FlxColor.RED];
		...
		var spr:FlxSprite = new FlxSprite(note.x + (GRID_SIZE * 0.5) - 4, note.y + GRID_SIZE / 2).makeGraphic(8, height, colorList[note.noteData]);

...
class SoundFrontEnd
{
	...
	private var changeVolumeHoldTime:Float = 0;
	/**
	...
	function update(elapsed:Float):Void
	{
		...
		if (FlxG.keys.anyJustPressed(muteKeys) || FlxG.keys.anyJustPressed(volumeUpKeys) || FlxG.keys.anyJustPressed(volumeDownKeys)) changeVolumeHoldTime = 0;
		
		if(FlxG.keys.anyPressed(volumeUpKeys) || FlxG.keys.anyPressed(volumeDownKeys))
		{
			final checkLastHold:Int = Math.floor((changeVolumeHoldTime - 0.35) * 10);
			changeVolumeHoldTime += elapsed;
			final checkNewHold:Int = Math.floor((changeVolumeHoldTime - 0.35) * 10);

			if(changeVolumeHoldTime > 0.35 && checkNewHold - checkLastHold > 0)
				changeVolume((checkNewHold - checkLastHold) * (FlxG.keys.anyPressed(volumeUpKeys) ? 1 : -1) / 20, false);
		}
		#end
	}
